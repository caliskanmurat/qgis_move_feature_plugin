...
class MoveFeature():
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg.pushButton_input_shp_4.clicked.connect(lambda x:self.getClickedCoor("start"))
            self.dlg.pushButton_input_shp_5.clicked.connect(lambda x:self.getClickedCoor("end"))
        ...
        if result:
            ...
            if self.method == 0:
                ...
                self.geomStart = self.startLayer.getFeature(0).geometry()
                
                self.endName = self.dlg.comboBox_2.currentText()
                self.endLayer = self.layers[self.endName]                
                self.geomEnd = self.endLayer.getFeature(0).geometry()          
